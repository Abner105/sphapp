  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userCheckAddress.fullAddress }}</span>
        收货人：<span>{{ userCheckAddress.consignee }}</span>
        <span>{{ userCheckAddress.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    submitOrder() {
      let data = {
        consignee: this.userCheckAddress.consignee,
        consigneeTel: this.userCheckAddress.phoneNum,
        deliveryAddress: this.userCheckAddress.fullAddress,
        paymentWay: "ONLINE",
        orderComment: this.comments,
        orderDetailList: this.orderInfo.detailArrayList,
      };
      // 提交订单接口
      this.$API
        .submit(this.orderInfo.tradeNo, data)
        .then((res) => {
          if (res.code == 200) {
